...
def parts(line):
    ...
    # A numbered instruction label is box structure, not script. It arrives
    # here as the remainder after its own quote is stripped, so is_header
    # never sees it, and four of them were being counted as authored lines.
    if re.match(r"^\d+\.\s+\S.*:$", rest):
        rest = ""
    if len(rest) > 20:
        got.append(rest)
    return got
...
CH_TITLES = {
    26: "The Renewal and the Price Increase",
    27: "Procurement and the Committee You Can't See",
    28: "Claims, Disputes, and Escalation",
    29: "Negotiating Internally",
}


def sheet(paths):
    """Emit the Part V review sheet: every authored counterpart-facing line,
    grouped by chapter, with a box to mark. AUTHORED-LINES.md explains why
    these exist and what the options are. This is the list itself, so Dan
    can rule on them without running anything."""
    exact, wild = catalog_patterns()
    ...
    for path in paths:
        ...
        for line in box_lines(path):
            ...
            for q in parts(line):
                ...
                if "______" in q and ok(q.replace("______", "something")):
                    continue
                found.setdefault(n, []).append(q)

    total = sum(len(v) for v in found.values())
    print("# Part V: the authored lines, for Dan to rule on")
    print()
    print("Generated by `python3 book/tools/verifylines.py --sheet "
          "book/chapters/ch2[6-9]a*.md`. Regenerate after any edit.")
    print()
    print("`book/AUTHORED-LINES.md` explains why these exist and lays out the")
    print("three options. This is the list itself. The catalog has no renewal,")
    print("procurement, claims or internal family, so these four chapters were")
    print("written rather than assembled. They are the only counterpart-facing")
    print("lines in the manual that are not your shipped copy.")
    print()
    print("**How to use it.** Read down the column. Mark each one:")
    print()
    print("- **K**, keep. It is close enough to what you already say.")
    print("- **R**, replace. Write the line you actually use next to it.")
    print("- **S**, seed. Good enough to belong in the catalog and the app.")
    print()
    print(f"{total} lines across four chapters. Most people can do a chapter in")
    print("ten minutes reading out loud, which is also the test: a line you")
    print("stumble on saying is a line to replace.")
    print()
    for n in sorted(found):
        print("---")
        print()
        print(f"## Chapter {n}. {CH_TITLES.get(n, '')}")
        print()
        print(f"{len(found[n])} lines.")
        print()
        print("| | Line | K / R / S | If R, what you say |")
        print("|---|---|---|---|")
        for i, q in enumerate(found[n], 1):
            cell = q.replace("|", "\\|")
            print(f"| {i} | {cell} | | |")
        print()
    return 0
...
if __name__ == "__main__":
    args = sys.argv[1:]
    mode = sheet if "--sheet" in args else drift if "--drift" in args else main
    args = [a for a in args if a not in ("--drift", "--sheet")]
    if not args:
        import glob
